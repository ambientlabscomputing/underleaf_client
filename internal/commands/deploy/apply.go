...
var (
	...
	applyOutput      string
	applyExpose      string // format: "service_name:port"
)
...
Examples:
  ...
  ufctl deploy apply my-app.json --state-dir ./state --report-dir ./reports`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// Handle --expose flag if provided
		if applyExpose != "" {
			if err := applyExposeFlag(deployment, applyExpose, deps); err != nil {
				deps.Printer.PrintError(fmt.Sprintf("Failed to apply expose flag: %v", err))
				return err
			}
		}
		...
		if hasExposures {
			deps.Printer.PrintInfo("🌐 Public Exposures (will be configured via server_api):")
			for _, service := range deployment.Services {
				if service.Expose != nil {
					hostname := service.Expose.Hostname
					if hostname == "" {
						hostname = "<auto-generated>"
					}
					deps.Printer.Print(fmt.Sprintf("   - %s:%d → %s", service.Name, service.Expose.Port, hostname))
				}
			}
			deps.Printer.Print("")
		}
		...
		if result.Success {
			...
			// Show exposures if any are configured
			displayExposuresSummary(deployment, deps)
		} else {
			deps.Printer.Print(lipgloss.NewStyle().
				Bold(true).
...
func init() {
	...
	ApplyCmd.Flags().StringVar(&applyExpose, "expose", "", "Expose a service publicly (format: service_name:port)")
}
...
// applyExposeFlag parses and applies the --expose flag to the deployment
func applyExposeFlag(deployment *types.AppDeployment, exposeFlag string, deps *utils.DependencyManager) error {
	// Parse format: "service_name:port"
	parts := strings.Split(exposeFlag, ":")
	if len(parts) != 2 {
		return fmt.Errorf("invalid expose format: expected 'service_name:port', got '%s'", exposeFlag)
	}
	...
	// Find service and apply expose config
	found := false
	for i, service := range deployment.Services {
		if service.Name == serviceName {
			found = true
			deployment.Services[i].Expose = &types.ExposeConfig{
				Port: port,
			}
			deps.Printer.PrintSuccess(fmt.Sprintf("✅ Expose applied: %s:%d", serviceName, port))
			break
		}
	}

	if !found {
		return fmt.Errorf("service '%s' not found in deployment", serviceName)
	}

	return nil
}

// displayExposuresSummary shows a table of exposures configured in the deployment
func displayExposuresSummary(deployment *types.AppDeployment, deps *utils.DependencyManager) {
	...
	// Display exposures section
	deps.Printer.Print("\n🌐 Public Exposures (via server_api):")
	deps.Printer.Print("   These exposures will be created when this deployment is applied via the server_api.")
	deps.Printer.Print("")
	deps.Printer.Print("   SERVICE           PORT    HOSTNAME")
	deps.Printer.Print("   ───────────────────────────────────────────────────────")
	for _, exp := range exposures {
		service := exp["service"].(string)
		port := exp["port"].(int)
		hostname := exp["hostname"].(string)
		deps.Printer.Print(fmt.Sprintf("   %-16s  %-6d  %s", service, port, hostname))
	}
	deps.Printer.Print("")
	deps.Printer.Print("   To create these exposures:")
	deps.Printer.Print("   1. Push this deployment to server_api (e.g., via web UI or API)")
	deps.Printer.Print("   2. The exposures will be created automatically with public URLs")
	deps.Printer.Print("   3. You can manage exposures with: ufctl expose list/get/revoke")
}
